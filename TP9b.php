...
$host = 'localhost';
$dbname = 'c60b1web'; // BDD_ tranquilité vacance sur ISPCONFIG
$user = 'c60b1web';  // uTILISATEUR rOUT DE LA BDD 
$pass = 'Yazakimoctar';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 2. Traitement Connexion (Utilisateur ou Admin)
    ...
    elseif ($origine == "Surveillance") {
        $sql = "INSERT INTO DEMANDE (date_debut, date_fin, contact_nom, contact_tel, id_utilisateur) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
    ...
    elseif ($origine == "Affecter") {
        $stmt = $pdo->prepare("UPDATE DEMANDE SET agent_affecte = ? WHERE id_demande = ?");
...
<body>
    ...
    <img src="Untitled.png" alt="Logo Z" class="logo">
    ...
    <div class="container <?php echo ($etape == 'admin_panel') ? 'admin-container' : ''; ?>">
        ...
        <?php if ($etape == "inscription"): ?>
            ...
            <p style="text-align:center;">Déjà inscrit ? <form action="" method="POST"><button type="submit" name="origine" value="AllerConnexion" style="background:#3498db;">Se connecter</button></form></p>
        <?php endif; ?>
        ...
        <?php if ($etape == "admin_panel"): ?>
            <h2>PLANNING DE SURVEILLANCE DES AGENTS</h2>
            ...
            <table>
                ...
                <?php
                $reponse = $pdo->query("SELECT d.*, u.nom, u.prenom, u.adresse FROM DEMANDE d JOIN UTILISATEUR u ON d.id_utilisateur = u.id_utilisateur");
            ...
            <br><a href="tp9b.php">Déconnexion</a>
        <?php endif; ?>

        <?php if ($etape == "termine"): ?>
            <h2>Merci !</h2>
            <p>Votre demande a été enregistrée. Nos agents passeront durant votre absence.</p>
            <a href="tp9b.php">Retour à l'accueil</a>
        <?php endif; ?>
